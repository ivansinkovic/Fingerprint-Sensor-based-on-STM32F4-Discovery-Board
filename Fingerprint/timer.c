...
uint32_t timer2_Ticks_Microsec; 
...
void timer2_init(void) { 
 ...
 //back to 0  
 TIM_TimeBaseStructure.TIM_Period = TimerPeriod/1000;  //microseconds!!!
...
{  
	...
	{   
		...
		timer2_Ticks_Microsec++;  
	} 
}

uint32_t timer2_get_microsec()
{ 
uint32_t value;
		NVIC_DisableIRQ(TIM2_IRQn);
		value = timer2_Ticks_Microsec;
...
{ 
uint32_t t1, t2;
	t1 = timer2_get_microsec();
  while(1)  
	{   
		t2 = timer2_get_microsec();
		if ((t2 - t1) >= us) break;
